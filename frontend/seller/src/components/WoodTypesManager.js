...
function WoodTypesManager() {
  ...
  const handleAddType = useCallback(async (e) => {
    ...
    // Validation
    const newTypeNameOriginal = newType.neme.trim(); // Keep original for creation
    const newTypeNameNormalized = newTypeNameOriginal.toLowerCase();

    if (!newTypeNameOriginal) {
      alert(SELLER_TEXTS.WOOD_TYPE_NAME_REQUIRED_ERROR);
      return;
    }

    // Check for duplicate wood type name (case-insensitive, ignoring leading/trailing whitespace)
    // Ensure woodTypes and woodTypes.data are available before trying to access them
    if (woodTypes && woodTypes.data && Array.isArray(woodTypes.data)) {
      const existingTypeNamesNormalized = woodTypes.data.map(type => type.neme.trim().toLowerCase());
      if (existingTypeNamesNormalized.includes(newTypeNameNormalized)) {
        alert(SELLER_TEXTS.WOOD_TYPE_ALREADY_EXISTS_ERROR);
    ...
    try {
      ...
      // Use the original trimmed name for creation, not the normalized one.
      const woodTypeResponse = await mutate(() => apiService.createWoodType({
        neme: newTypeNameOriginal,
      ...
      alert(`Wood type "${newTypeNameOriginal}" created successfully!`);
  ...
  }, [newType, woodTypes, mutate, refetchTypes, refetchPrices, resetForm, SELLER_TEXTS.WOOD_TYPE_ALREADY_EXISTS_ERROR, SELLER_TEXTS.WOOD_TYPE_NAME_REQUIRED_ERROR, SELLER_TEXTS.VALID_INITIAL_PRICE_ERROR]);
  ...
  const handleUpdateType = useCallback(async (e) => {
    ...
    const updatedNameOriginal = editingType.neme.trim();
    if (!updatedNameOriginal) {
      alert(SELLER_TEXTS.WOOD_TYPE_NAME_REQUIRED_ERROR);
      return;
    }
    const updatedNameNormalized = updatedNameOriginal.toLowerCase();

    if (woodTypes && woodTypes.data && Array.isArray(woodTypes.data)) {
      for (const type of woodTypes.data) {
        if (type.id === editingType.id) {
          continue; // Skip self-comparison
        }
        const existingTypeNameNormalized = type.neme.trim().toLowerCase();
        if (existingTypeNameNormalized === updatedNameNormalized) {
          alert(SELLER_TEXTS.WOOD_TYPE_ALREADY_EXISTS_ERROR);
          return;
        }
      }
    ...
    try {
      await mutate(() => apiService.updateWoodType(editingType.id, {
        neme: updatedNameOriginal, // Use trimmed original name
        description: editingType.description.trim() || null // Also trim description
      }));
      setEditingType(null);
      refetchTypes();
      // Optionally, show a success message for update
      // alert(`Wood type "${updatedNameOriginal}" updated successfully!`);
    } catch (err) {
      console.error('Failed to update wood type:', err);
      let errorMessage = 'Failed to update wood type. Please try again.';
      if (err.response?.data?.detail) {
        errorMessage = err.response.data.detail;
      } else if (err.message) {
        errorMessage = err.message;
  ...
  }, [editingType, woodTypes, mutate, refetchTypes, SELLER_TEXTS.WOOD_TYPE_ALREADY_EXISTS_ERROR, SELLER_TEXTS.WOOD_TYPE_NAME_REQUIRED_ERROR]);
