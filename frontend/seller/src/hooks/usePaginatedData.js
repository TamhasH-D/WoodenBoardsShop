 ...
 */
export function useCompleteDataset(fetchFunction, dependencies = [], options = {}) {
 ...
 */
export function useProgressiveData(fetchFunction, dependencies = [], options = {}) {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [total, setTotal] = useState(0);
 ...
 */
export function usePaginatedData(fetchFunction, dependencies = [], options = {}) {
  const {
    initialPage = 0,
    pageSize = 20,
    ...otherOptions
  ...
  }, [fetchFunction, page, pageSize, ...dependencies]);
