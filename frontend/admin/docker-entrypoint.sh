...
# Detect current hosts for Admin Frontend
detect_admin_host() {
    if [ "$NODE_ENV" = "production" ]; then
...
CURRENT_HOST=$(detect_admin_host)
CURRENT_KEYCLOAK_HOST=$(detect_keycloak_host)
echo "📍 Using admin frontend host: $CURRENT_HOST"
...
# Build OIDC URLs for Admin
REACT_APP_OIDC_AUTHORITY="${KEYCLOAK_PROTOCOL}://${CURRENT_KEYCLOAK_HOST}${KEYCLOAK_PORT_SUFFIX}/realms/AdminRealm"
REACT_APP_OIDC_CLIENT_ID="admin-frontend"
