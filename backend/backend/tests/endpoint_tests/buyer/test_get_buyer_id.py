...
async def test_get_buyer_by_id(
    ...
    response = await client.get(URI.format(buyer_id=buyer.keycloak_uuid))
    assert response.status_code == 200

    response_data = response.json()["data"]
    assert response_data["keycloak_uuid"] == str(buyer.keycloak_uuid)
    assert response_data["created_at"] == buyer.created_at.isoformat()
    assert response_data["updated_at"] == buyer.updated_at.isoformat()
